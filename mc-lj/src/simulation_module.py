...
def simulation(temperature, density, steps, burnin_steps, param_interact=None, param_sys=None, mc_par_over_box=0.1):
    import copy
...
# run montecarlo until burnin without saving values
    ...
    for i in range(int(steps - burnin_steps)):
        ...
        if i % ((steps - burnin_steps)/10) == 0:
            ...
            # print(f'{np.abs((E[-1]-interact.compute_energy(my_sys.positions,my_sys.box)/my_sys.positions.shape[1])/E[-1])*100:.2e}')
